...
public class UsuarioDAO {
    ...
    public void RegistrarTransfTed(UsuarioDTO objUsuarioDTO){
        ...
        try {
            ...
            saldoResultado.close();
            pstmSaldo.close();
            float valorpix = objUsuarioDTO.getId_valorTransTED();
    ...
    public void ImprimeBDTD(UsuarioDTO objUsuarioDTO){
        ...
        String sqlArmazenaTD = "INSERT INTO `grupo4historico` (`CPF_REMET`,`VALOR`, `CONTA_REMET`, `CONTA_DEST`, `AGENCIA _REMET`, `AGENCIA_DEST`, `TIPO_TRANS`) VALUES (?, ?, ?, ?, ?, ?, ?)";
        conn = new Conexao().conectaDB();
    ...
    public ArrayList<UsuarioDTO> HistoricoPIX(){ 
        
        conn = new Conexao().conectaDB();
        String userInput = JOptionPane.showInputDialog("Digite novamente o cpf: ");
        String sql = "SELECT * FROM grupo4historico WHERE CPF_REMET =" + userInput;
        try{        
            pstm = conn.prepareStatement(sql);
            rs = pstm.executeQuery();
            ...
            while(rs.next()){
                ...
                objUsuarioDTO.setId_agencia(rs.getInt("AGENCIA _REMET"));
